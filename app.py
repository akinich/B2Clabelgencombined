...
def create_pdf(data_list, font_name, width, height, font_override=0, selected_columns=[]):
    buffer = BytesIO()
    ...
    if selected_columns:
        ...
        if st.button("Generate PDF"):
            if not data_list:
                ...
                pdf_buffer = create_pdf(data_list, selected_font, width_mm*mm, height_mm*mm, font_override, selected_columns)
                st.download_button(
                    label="Download PDF",
                    data=pdf_buffer,
